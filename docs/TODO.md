...
- [ ] 2.1 实现 `src/components/NavControls.tsx`
  - 上一页/下一页按钮
  - 禁用状态样式（第一张禁用上一页，最后一张禁用下一页）
- [ ] 2.2 实现 `src/components/ProgressBar.tsx`
  - 顶部进度条
  - 显示当前页码 (如 3/17)
- [ ] 2.3 实现 `src/hooks/useKeyboardNav.ts`
  - `←` / `→` 箭头键导航
  - `Space` 下一页
  - `Home` 跳到第一张
  - `End` 跳到最后一张
- [ ] 2.4 实现 `src/hooks/useHashSync.ts`
  - URL hash 同步 (#slide-1, #slide-2, ...)
  - 刷新页面保持当前幻灯片位置
- [ ] 2.5 在 App.tsx 中集成所有导航组件
- [ ] 2.6 添加测试幻灯片 (02-05) 以验证导航

### 验收标准

| 标准 | 验收方式 |
|------|----------|
| 点击上一页/下一页按钮可切换幻灯片 | 点击按钮，观察页面变化 |
| 键盘 `←` `→` 可导航 | 按键测试 |
| `Space` 跳到下一页 | 按键测试 |
| `Home` 跳到第一张，`End` 跳到最后一张 | 按键测试 |
| URL 显示 `#slide-N` 格式 | 观察 URL 栏 |
| 刷新页面保持在当前幻灯片 | 在第 3 张刷新，验证仍显示第 3 张 |
| 进度条正确显示当前位置 | 目视检查进度条和页码 |
| 第一张时上一页按钮禁用，最后一张时下一页按钮禁用 | 导航到边界检查按钮状态 |
